...
const BidirDshotTelemetryType telemetryTypeLut[16] = {
	BidirDshotTelemetryType::ERPM, BidirDshotTelemetryType::ERPM, BidirDshotTelemetryType::TEMPERATURE,
	BidirDshotTelemetryType::ERPM, BidirDshotTelemetryType::VOLTAGE, BidirDshotTelemetryType::ERPM,
	BidirDshotTelemetryType::CURRENT, BidirDshotTelemetryType::ERPM, BidirDshotTelemetryType::DEBUG_FRAME_1,
	BidirDshotTelemetryType::ERPM, BidirDshotTelemetryType::DEBUG_FRAME_2, BidirDshotTelemetryType::ERPM,
	BidirDshotTelemetryType::STRESS, BidirDshotTelemetryType::ERPM, BidirDshotTelemetryType::STATUS,
...
BidirDShotX1::BidirDShotX1(uint8_t pin, uint32_t speed, PIO pio, int8_t sm) {
	...
	if (sm >= 4 || sm < -1 ||
		...
		DEBUG_PRINTF(
			"Invalid parameters: Check that sm is -1...3, pin is 0...29 (or 0...47 on RP2350), speed is 150...4800 and pio is pio0 or pio1 (or pio2 on RP2350). You supplied: sm=%d, pin=%d, speed=%d, pio=%s\n"
			, sm, pin, speed, pioStr);
		iError = true;
		return;
	}
	...
	if (speed != 300 && speed != 600 && speed != 1200 && speed != 2400) {
		DEBUG_PRINTF(
			"Unofficial speed: %d. Unless you know what you are doing, please select DShot 300, 600, 1200 or 2400.\n",
			speed);
	}
...
void BidirDShotX1::sendSpecialCommand(uint16_t cmd) {
	for (int i = 0; i < 10; i++) {
		this->sendRaw11Bit(cmd);
